...
| validator-architecture | Architecture.md | Consistency with docs/Architecture.md |
...
2. Use swarm of agents and specialized skills to deliver:
   ...
   - `@architect` — ensure consistency with docs/Architecture.md
   - Implementation agents — parallel Task tool for independent modules
3. **Monorepo scope:** spawn separate tasks for `apps/api` and `apps/tma` when both affected
4. **Make implementation modular** for reuse in other parts of the monorepo
5. Save frequent commits to GitHub
6. Spawn concurrent tasks to speed up development

**Implementation rules:**
- Each package gets its own Task for parallel execution
- Reference SPARC docs, don't hallucinate code
- Commit after each logical unit: `feat(<scope>): <what>`
- Run tests in parallel with implementation
- All amounts in kopecks (integers), never floats
...
| Agent | Scope | Focus |
|-------|-------|-------|
| code-quality | Source code | Clean code, TypeScript strict, naming |
| architecture | Integration | Consistency with project architecture |
| security | Security surface | initData validation, Zod schemas, SQL injection |
| performance | Hot paths | LLM timeout, Redis cache, bottlenecks |
...
3. Save frequent commits: `fix(<scope>): <what>`
4. Benchmark after implementation (especially LLM response times)

...
description: Intelligent feature implementation pipeline. Analyzes complexity and selects
  ...
  and frequent commits. $ARGUMENTS: feature name, ID, or brief description (optional)
---
...
| Has external API integration (YandexGPT, ЮKassa) | +2 |
...
| -1 to +9 | `/feature` | Standard feature, needs SPARC lifecycle |
...
3. Use `Task` tool to parallelize independent package changes
...
2. Spawn concurrent tasks for independent packages:
   - Parallel test writing + implementation
   - Parallel apps/tma + apps/api when independent
...
2. Run `/next <feature-id>` to cascade dependency unblocking
3. Commit roadmap: `git add .claude/feature-roadmap.json && git commit -m "docs(roadmap): mark <feature> as done"`
...
- Spawn concurrent agents when feature touches both apps/api and apps/tma
